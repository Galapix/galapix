...
{
public:
  ZoomifyTileJob(JobHandle const& job_handle, URL const& url,
                 int scale, Vector2i const& pos,
  ...
  void run() override
  {
    try
    {
      ...
      {
        surf::SoftwareSurface surface = surf::jpeg::load_from_file(m_url.get_stdio_name());
